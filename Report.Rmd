...
Then we model data with Caret package using Random Forest algorithm.
```{r TranformData, echo = TRUE}
controlRf <- trainControl(method="cv", 5)
caretmodelRf <- train(classe ~ ., data=TrainSet, method="rf", trControl=controlRf, ntree=50)
caretmodelRf
predict(caretmodelRf, TestDataClean)
saveRDS(caretmodelRf, "caretmodelRF.RDS")
```
...
mean(predict(caretmodelRF, TestSet) == TestSet$classe) * 100
```
...
Then we perform cross validation on Random Forest model 
```{r , echo = TRUE}
modelRF = readRDS("modelRF.RDS")
predictRf <- predict(modelRF, TestSet)
predict(modelRF, TestDataClean)
